...
(1) 각 정점 쌍의 최단 거리를 기록하는 `ans` 배열 외에, 각 정점에서 최단 거리로 가려면 **당장** 어느 정점으로 움직여야 하는지만을 기록하는 `nxt` 배열이 하나 더 필요하다. 개시 시 각 노드 간의 연결이 `parent->child, weight`로 주어졌을 때 `ans` 배열에는 거리(weight)를, `nxt` 배열에는 child 자체를 기록한다. **nxt 배열은 그 정점에서 당장 이동할 수 있는 정점을 가리키게 된다.** 한번에 갈 수 없다면, 아무런 값도 기록되지 않은 채 넘어간다. 
