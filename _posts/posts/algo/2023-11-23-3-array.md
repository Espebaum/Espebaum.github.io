...
- 배열 내에서 합이 13이 되는 두 수를 찾기 위해서 배열을 순회할 때, 예를 들어 예제로 주어진 배열의 5에서는, 8을 확인한다. 해당 수의 짝이 되는(합이 13이 되는) 다른 인덱스의 수를 확인하는 방법은 숙지해두는게 좋은 것 같다. (메모의 두 포인터가 이 기술의 대략적인 명칭같다).
