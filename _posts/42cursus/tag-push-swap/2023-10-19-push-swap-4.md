...
- 예를 들어 capacity가 8이고, arr = {1, 2, 3, 4, 5, 6, 7, 8}인 배열을 담고 있는 스택 a를 생각해보자. `arr[head]`는 1이고, `arr[tail - 1]`은 8이다. head와 tail은 둘다 0이다. 기억나는가? [tail이 0일 때의 처리](https://espebaum.github.io/42cursus/push-swap-3.html#4-int-backt_deque-a). 
