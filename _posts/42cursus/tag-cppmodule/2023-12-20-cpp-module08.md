...
- cbegin과 crbegin 그리고 cend와 crend를 구현할 때, c++11함수를 가져다쓰는 실수를 하면 안된다. 제대로 확인하지 않았다가 예리한 사람한테 걸리면 그대로 fail을 받게 된다. 예를 들어 `std::deque::begin`의 reference를 살펴보자.
...
- 그러나 cbegin은 빼박 c++11함수여서 const iterator, const reverse iterator의 cbegin, cend, crbegin, crend를 호출한답시고 `this->c.crbegin()` 이라도 외치는 순간 바로 c++11함수를 사용하게 된다. 상수 반복자를 구현할 때 cbegin, cend 함수를 바로 사용하는 것이 아니라, 상수 반복자를 반환받는 방식으로 c++11 함수 사용을 회피해야 한다. 이것으로 모듈 8을 마친다.