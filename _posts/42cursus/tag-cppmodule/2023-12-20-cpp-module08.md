...
- 그러나 cbegin은 빼박 c++11 함수여서 const iterator, const reverse iterator의 cbegin, cend, crbegin, crend를 호출한답시고 `this->c.cbegin()` 이라도 외치는 순간 바로 c++11함수를 사용하게 된다. 상수 반복자를 구현할 때 cbegin, cend 함수를 바로 사용하는 것이 아니라, 상수 반복자를 반환받는 방식으로 c++11 함수 사용을 회피해야 한다. 이것으로 모듈 8을 마친다.