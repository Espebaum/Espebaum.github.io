...
- 이 코드는 파생 클래스 `ShrubberyCreationForm`의 대입 연산자 오버로딩 함수이다. 먼저 기반 클래스를 초기화하기 위해 기반 클래스 AForm의 대입 연산자 오버로딩 함수를 호출한다. 물론 그 매개변수는 ShrubberyCreationForm의 참조 ref(`const ShrubberyCreationForm &ref`)이다. 파생 클래스 객체는 문제 없이 기반 클래스의 매개변수로 사용될 수 있다 (다형성의 기본 원리). 그러나 `AForm::operator=`의 반환값은 AForm의 참조(`AForm&`)이기 때문에 업 캐스팅되어 AForm&으로 반환된다.
...
- 한눈에 보자면 `const ShruberryCreationForm& ref`가 AForm의 할당 대입 연산자의 매개 변수로 들어가 AForm&으로 반환되고(`AForm::operator=(ref)`), 그 AForm&이 dynamic_cast되어(원래 shrubbery였으니까 가능) ShruberryCreationForm&으로 반환되는 3단 변신 코드다.
