...
(4) **이제 작은 값 배열의 숫자를 큰 값 배열에 삽입하면서 정렬한다.** 이때 순서대로 집어넣는게 아니라 Jacobstahl수를 따르면서 삽입한다. 이때를 위해 만들어둔 Jacobstahl수 배열을 꺼내온다. 지금 정렬하려는 배열이 6개라서 준비된 Jacobstahl 배열도 6개인데, 배열이 길어지면 Jacobstahl 배열 그 size에 맞도록 설정했다.
