...
- 수식을 파싱하여 숫자라면 스택에 추가하고, 연산자라면 스택에서 숫자를 제거하여 해당 연산을 수행한 이후 다시 스택에 넣는다. 모든 연산이 끝난 후에는 스택에 최종적인 연산의 결과값만이 남아있어야 한다. 또 연산자 하나에 스택의 top의 원소와 그 아래 숫자의 연산이 실행되고 그 결과값이 다시 스택에 push되기 때문에, 숫자의 개수는 연산자의 개수보다 하나가 많아야 한다. 이것을 체크하기 위해 연산자가 들어왔을 때 최소한 스택의 숫자가 2개 들어있는지, 모든 연산이 실행되었을 때 스택의 size가 1인지 등을 체크해야 할 것이다.
...
- 엉성한 구현이기는 하지만, 본인이 과제를 해결한 대략적인 절차를 설명하면서 모듈 9를 마무리하도록 하겠다. 예시로 정렬한 배열은 {21, 15, 34, 56, 41, 44}이다. 

(1) 매개 변수로 배열을 받아 배열이 모두 양의 정수로 이루어져 있는지 검사하고, 만약 그렇다면 Jacobsthal 수로 이루어져 있는 배열을 만든다. Jacobstahl수로 이루어진 배열의 size는 매개변수로 주어진 배열의 size와 동일하다. 그러면 Jacobstahl 수가 무조건 매개변수로 주어진 배열의 size를 넘게 된다. 
...
- Jacobstahl수의 배열을 채우는 함수다. 수식을 생각해내는게 꽤 힘들었던 기억이 난다.
<br> 
...
(3) 배열을 두 개로 나눈다. 하나는 바로 위 사진에서 "작은 값" 만으로 이루어진 배열이고, 나머지는 "큰 값" 만으로 이루어진 배열이다. 여기까지가 "병합-삽입" 중의 "병합"이다. 이후의 작업은 삽입이다. (배열이 홀수 개라면 나머지 하나는 그냥 작은 값 배열의 끄트머리에 넣어버린다)

<center><img src="/assets/img/cpp_module/09-03.png" width="100%" height="100%"></center><br>

 진짜 제대로 된 포드-존슨 알고리즘이라면 여기까지의 과정이 재귀로 이루어져 한다는 것으로 생각하고 있다. 병합 정렬을 하듯이 배열을 둘로 나누어 더 이상 나눌 수 없을 때까지 나눈 후, 배열을 합치면서 이렇게 배열을 둘로 나누면 되지 않을까? 하고 생각한다. ~~생각만 해본다.~~

(4) **이제 작은 값 배열의 숫자를 큰 값 배열에 삽입하면서 정렬한다.** 이때 순서대로 집어넣는게 아니라 Jacobstahl수를 따르면서 삽입한다. 이때를 위해 만들어둔 Jacobstahl수 배열을 꺼내온다.
...
(5) 삽입할 때, 반복문을 돌지 않고 이진 탐색을 실행한다. 큰 값 배열은 언제나 오름차순으로 정렬되어 있기 때문에 별도의 절차 없이 바로 이진 탐색을 실행하면 된다(큰 값 배열은 정렬되어 있지 않다). 이때 Jacobstahl수 순서대로 정렬하면 이진 탐색의 수를 줄일 수 있다. **줄일 수도 있다는 게 포인트다, 무조건 줄어드는 게 아니다.** 직접 경우의 수를 확인해보자. 
...
- 1번의 탐색으로 b1(15)을 정렬할 수가 있다. 이진 탐색은 탐색 범위를 절반으로 나누면서 실행하는데, 이미 탐색 범위가 1이기 때문에 이진 탐색 1번으로 적절한 위치를 찾을 수 있다. Jacobstahl 순서를 따른다면, 다음으로 정렬할 수는 52다. **b3이 a3보다 항상 작다는 것을 고려했을 때** 52의 경우의 수는 아래와 같다.
...
- **b2는 a2보다 작기 때문에**, 3개의 경우의 수를 가진다. 이 경우 이진 탐색을 2번 실행하여 33의 위치를 찾을 수 있다. 따라서 Jacobstahl 순서를 따를 경우, 5번의 이진 탐색으로 배열을 정렬할 수 있다.  

(5 - 2) Jacobstahl 순서를 따르지 않는 경우

- b1을 a에 넣는것 까지는 똑같다. 그 다음으로 38을 정렬한다. 경우의 수는 아래와 같다.

<center><img src="/assets/img/cpp_module/09-12.png" width="80%" height="80%"></center><br>

- 경우의 수가 3이므로, 이진 탐색 2번으로 찾을 수 있다. 마지막으로 44를 정렬한다.

<center><img src="/assets/img/cpp_module/09-13.png" width="80%" height="80%"></center><br>

- 탐색 범위가 5이므로, 이진 탐색 3번으로 찾을 수 있다. Jacobstahl 순서를 따르지 않는 경우, 6번의 이진 탐색의 배열을 정렬할 수 있다.
...
- 연속적인 메모리 공간에 저장(물리적으로 연속적)
- iterator, index로 접근 가능
- 동적으로 확장 가능한 Dynamic Array로 구현
- 물리적으로 연속되어 있기 때문에 개별 원소에 대한 접근 속도가 빠름, 끝에서 삽입/제거하는 속도가 가장 빠름.

- 단점

- 동적으로 확장할 때, 재할당 비용이 꽤 큼
- Capacity를 확장시켜줄수 있는 적절한 크기의 reserve가 중요(메모리 확보)
...
- random access iterator를 통한 index 접근 가능
- Dynamic Array로 구현
- vector와 비슷한 점이 많지만 컨테이너 끝 뿐만 아니라 첫 부분의 삽입/제거도 효율이 높다.
- 그리고 가장 큰 차이점은 **연속된 메모리에 올라가 있지 않다**. 몇 바이트 단위의 **chunk로 쪼개져 있으며** 이 chunk는 내부적으로 관리된다.
...
- Double Linked list, 이중 연결 리스트 기반 구현
- 어느 위치에서도 삽입/제거가 빠르다

- 단점 & 주의

- index로 접근 불가(처음 또는 끝에서 선형 탐색해야함)
- 컨테이너 내 원소 순회는 forward/reverse 순회만 가능하며 느림
- 상호 연결 정보를 위해 추가 메모리 사용

### 결론

- 그런데 **무조건 줄어드는 게 아니라고** 했다. 운이 좋아서 줄어든거지, 작은 값 배열이 어떤 식으로 배치되어 있느냐에 따라 이진 탐색 횟수가 줄어들지 않을 수도 있다. 그리고 이진 탐색 횟수를 줄이는데 큰 역할을 했던 것은 Jacobstahl 순서로 정렬하는게 아니라 a1이 b1보다 크다는 사실(ak가 bk보다 크다는 사실)이었다. 여러모로 아리송한 exercise였고, 평가자들도 포드-존슨 알고리즘을 정확히 알지 못해 어물쩍 넘어가버렸던 기억이 있다. 그래도 푸는 재미가 있는 문제들이었다고 생각한다. 아래는 Jacobstahl 순서로 이진 탐색 + ak가 bk보다 크다는 사실이 적용된 이진 탐색 숫자와, 그것이 적용되지 않은 이진 탐색 숫자를 비교한 출력을 보여주면서 모듈을 끝내도록 하겠다.

(1) "Jacobstahl 순서로 이진 탐색 + ak가 bk보다 크다" 적용됨
