...
- 가상 함수를 통한 동적 바인딩으로 **subtype polymorphism(run time polymorphism)**을 구현할 수 있다. 어떤 방식으로 작동할까? 
...
- 근데 `base->print_d()`는 왜 안되는 걸까? 결국 base 포인터 안에 들어있는 본체는 Derived인데, print_d()를 가리킬 수 있는 것이 아닐까하는 생각이 들기 때문이다.